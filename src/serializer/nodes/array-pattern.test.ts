...
suite('ArrayPattern', () => {
  test('[ Elision(opt) AssignmentRestElement[?Yield, ?Await](opt) ]', () => {
    ...
    expect(serialize(ArrayPattern([null, RestElement(Identifier('a'))]))).toBe(
      '[,...a]',
    );
  });
  ...
  test('[ AssignmentElementList[?Yield, ?Await] , Elision(opt) AssignmentRestElement[?Yield, ?Await](opt) ]', () => {
    expect(
      serialize(
        ArrayPattern([
          Identifier('a'),
