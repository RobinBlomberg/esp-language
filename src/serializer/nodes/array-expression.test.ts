...
suite('ArrayExpression', () => {
  ...
  suite('[ ElementList[?Yield, ?Await] ]', () => {
    test('Elision(opt) AssignmentExpression[+In, ?Yield, ?Await]', () => {
      ...
      expect(serialize(ArrayExpression([null, Identifier('a')]))).toBe('[,a]');
    });

    test('Elision(opt) SpreadElement[?Yield, ?Await]', () => {
      expect(serialize(ArrayExpression([SpreadElement(Identifier('a'))]))).toBe(
        '[...a]',
      );
      expect(
        serialize(ArrayExpression([null, SpreadElement(Identifier('a'))])),
      ).toBe('[,...a]');
    });

    test('ElementList[?Yield, ?Await] , Elision(opt) AssignmentExpression[+In, ?Yield, ?Await]', () => {
      expect(
        serialize(ArrayExpression([Identifier('a'), null, Identifier('b')])),
      ).toBe('[a,,b]');
    });
