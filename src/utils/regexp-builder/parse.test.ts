  ...
  it('should work with helpers', () => {
    expect(
      Concatenation([
        ...
        Alternation([
          ...
          Concatenation([
            CharacterClass(false, [Range('1', '9')]),
            CharacterClass(false, [Range('0', '9')]).star(),
          ]),
        ]),
        End(),
      ]).regex(),
    ...
    expect(
      Concatenation([
        ...
        Group(false, [
          Group(false, [
            CharacterClass(true, [Character(' ')]).plus(),
            Character(' '),
            Character('('),
            CharacterClass(true, [Character(' ')]).plus(),
            Character(' '),
          ...
          Group(true, [CharacterClass(true, [Character(' ')]).plus()]),
          Character(' '),
          Character('(').optional(),
        ]).optional(),
        Group(true, [Any().plus().lazy()]),
        Character(':'),
        Group(true, [CharacterClass(false, [Range('0', '9')]).plus()]),
        Character(':'),
        Group(true, [CharacterClass(false, [Range('0', '9')]).plus()]),
      ]).regex(),
    ).toStrictEqual(
